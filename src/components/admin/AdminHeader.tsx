...
export const AdminHeader: React.FC<AdminHeaderProps> = ({ title, subtitle }) => {
  ...
  return (
    <>
      {/* Odoo / Skripsi ERP Main Top Navbar */}
      <nav className="o_main_navbar sticky top-0 z-40">
        <div className="flex items-center h-full gap-2 sm:gap-3 min-w-0">
          ...
          <button
            ...
            className="text-white hover:bg-white/15 px-2.5 h-full flex items-center justify-center transition-colors cursor-pointer relative rounded-sm group"
            title="App Switcher Matrix (Home)"
          >
            <Grid className="w-5 h-5 text-amber-300 group-hover:rotate-90 transition-transform duration-300" />
            {totalPendingNotifications > 0 && (
              <>
                <span className="absolute top-2 right-1.5 w-2 h-2 rounded-full bg-rose-500 animate-ping" />
                <span className="absolute top-2 right-1.5 w-2 h-2 rounded-full bg-rose-600" />
              </>
            )}
          </button>

          {/* Brand Logo & Active Role Identifier */}
          <div className="flex items-center gap-2 border-r border-white/20 pr-3 mr-1 h-7">
            <span className="font-extrabold tracking-tight text-white text-xs sm:text-sm flex items-center gap-1.5">
              <span className="text-amber-400">Circular</span>Trace
            </span>
            <span className="text-white/40 hidden sm:inline">|</span>
            <span className="hidden sm:inline-flex items-center gap-1 px-2 py-0.5 rounded text-[11px] font-bold bg-white/15 text-white border border-white/20">
            ...
            <div className="hidden lg:flex items-center h-full overflow-x-auto no-scrollbar">
              {moduleTabs.map((tab) => {
                ...
                return (
                  ...
                  >
                    <TabIcon className={`w-3.5 h-3.5 ${isActive ? 'text-amber-300' : 'text-white/70'}`} />
                    <span>{tab.label}</span>
                    {tab.badge > 0 && (
        ...
        {/* Right side System Tray: Live Clock, Search, Verified Node, Notifications, Role Switcher, Profile */}
        <div className="flex items-center gap-2 sm:gap-3 h-full">
          {/* Live Real-Time Clock (WIB) */}
          <div
            className="hidden xl:flex items-center gap-1.5 px-2.5 py-1 bg-white/10 hover:bg-white/15 rounded-md text-white text-xs font-mono select-none transition border border-white/10"
          ...
          <button
            ...
            className="hidden md:flex items-center gap-2 px-2.5 py-1 rounded-md bg-white/10 hover:bg-white/20 text-white/90 text-xs font-medium border border-white/15 transition-all cursor-pointer"
            title="Pencarian Cepat (Ctrl+K)"
          >
            <Search className="w-3.5 h-3.5 text-amber-300" />
            <span className="hidden lg:inline text-[11px]">Cari lot / ID...</span>
            <kbd className="text-[9px] font-mono bg-white/20 text-white px-1.5 py-0.5 rounded border border-white/20">
              ⌘K
            </kbd>
          </button>

          {/* Verified Node Badge */}
          <div className="hidden sm:flex items-center gap-1 px-2.5 py-1 rounded-md bg-emerald-500/20 border border-emerald-400/30 text-emerald-300 text-[11px] font-semibold">
            <ShieldCheck className="w-3.5 h-3.5 text-emerald-400" />
            <span className="hidden lg:inline">Node Terverifikasi</span>
          </div>
          ...
          <div className="relative">
            <button
              ...
              className="text-white hover:bg-white/15 px-2 py-1 rounded-md text-xs font-semibold flex items-center gap-1.5 border border-white/15 transition-colors cursor-pointer"
              title="Ganti Stasiun Kerja / Role"
            >
              <Building2 className="w-3.5 h-3.5 text-amber-300" />
              <span className="hidden sm:inline">{currentUser.organization || currentUser.name}</span>
            ...
            {roleMenuOpen && (
              <>
                ...
                <div className="absolute right-0 mt-1.5 w-60 bg-white rounded-xl shadow-xl border border-slate-200 p-2 z-50 animate-in fade-in slide-in-from-top-2 duration-150 text-slate-800">
                  <div className="px-2.5 py-1.5 text-[10px] uppercase font-bold text-slate-400 border-b border-slate-100">
                    Ganti Stasiun Rantai Pasok
                  </div>
          ...
          <div className="relative">
            <button
              ...
              className="text-white hover:bg-white/15 p-1.5 rounded-md relative transition-colors cursor-pointer"
            ...
            {notificationOpen && (
              <>
                ...
                <div className="absolute right-0 mt-2 w-80 sm:w-96 bg-white rounded-2xl shadow-xl border border-slate-200 p-4 z-50 animate-in fade-in slide-in-from-top-2 duration-150 text-slate-800">
                  <div className="flex items-center justify-between pb-3 border-b border-slate-100">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-bold text-xs text-slate-900">Log Ledger Transaksi</span>
                    </div>
                  ...
                  <div className="divide-y divide-slate-100 max-h-72 overflow-y-auto my-2">
                    {recentTransactions.map((trx) => (
                      <div key={trx.id} className="py-2.5 px-1 space-y-1 hover:bg-slate-50 rounded-lg transition-colors">
                        <div className="flex items-center justify-between text-[11px]">
                          <span className="font-bold text-slate-900 truncate">
                            {trx.fromName} → {trx.toName}
                          </span>
                          <span className="text-[10px] text-slate-400 font-mono">{trx.date}</span>
                        </div>
                        <p className="text-[11px] text-slate-600 truncate">{trx.itemName}</p>
                  ...
                  <button
                    ...
                    className="w-full mt-2 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-800 text-xs font-bold transition-colors text-center block"
                  >
          ...
          {/* Wallet Balance */}
          <div className="hidden 2xl:flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-white/10 text-amber-300 text-xs font-mono font-bold">
          ...
          <button
            ...
            className="px-3 py-1 rounded-md bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-xs transition-all shadow-2xs flex items-center gap-1.5 cursor-pointer"
            title="Buka Toko E-Commerce Publik"
          >
            <Store className="w-3.5 h-3.5" />
            <span className="hidden md:inline">Toko Publik</span>
          </button>
      ...
      {moduleTabs && (
        <div className="lg:hidden border-b border-slate-200 bg-white shadow-2xs px-3 py-2 flex items-center gap-2 overflow-x-auto no-scrollbar">
          {moduleTabs.map((tab) => {
            const TabIcon = tab.icon;
            const isActive = activeModuleTab === tab.id;
            return (
      ...
      {searchModalOpen && (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/70 backdrop-blur-xs flex items-start justify-center p-4 sm:pt-20 animate-in fade-in duration-150">
          <div className="relative bg-white rounded-2xl max-w-xl w-full shadow-2xl border border-slate-200 overflow-hidden">
            ...
            <div className="p-4 border-b border-slate-200 flex items-center gap-3">
              ...
              <input
                ...
                className="w-full text-sm font-medium focus:outline-hidden text-slate-900 placeholder-slate-400"
              />
              <button
            ...
            <div className="p-4 max-h-80 overflow-y-auto space-y-2">
              ...
              <div
                ...
                className="p-3 rounded-xl hover:bg-slate-50 border border-transparent hover:border-slate-200 cursor-pointer transition-colors flex items-center justify-between"
              >
              ...
              >
                ...
                <span className="text-xs text-emerald-600 font-bold">Buka →</span>
              </div>
            </div>
