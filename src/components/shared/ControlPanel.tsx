          ...
          {breadcrumbs.map((bc, idx) => (
            <React.Fragment key={idx}>
              ...
              {bc.onClick ? (
                <button
                  onClick={bc.onClick}
                  className="text-stone-600 hover:text-amber-800 font-semibold transition-colors cursor-pointer truncate"
                >
                  {bc.label}
                </button>
              ) : (
            ...
            <div className="relative">
              ...
              {isFilterDropdownOpen && (
                <>
                  ...
                  <div className="absolute left-0 mt-1.5 w-52 bg-white rounded-xl shadow-xl border border-stone-200 py-1 z-30 text-xs text-stone-700 animate-in fade-in">
                    ...
                    {filterOptions.map((opt) => (
                      <button
                        ...
                        className="w-full text-left px-3 py-2 hover:bg-stone-50 flex items-center justify-between font-medium cursor-pointer"
                      >
                        <span>{opt.label}</span>
                        {activeFilter === opt.id && <Check className="w-3.5 h-3.5 text-amber-700" />}
                      </button>
                    ))}
                  </div>
                </>
                ...
                <>
                  ...
                  <div className="absolute left-0 mt-1.5 w-52 bg-white rounded-xl shadow-xl border border-stone-200 py-1 z-30 text-xs text-stone-700 animate-in fade-in">
                    ...
                    {groupByOptions.map((opt) => (
                      ...
                      >
                        <span>{opt.label}</span>
                        {activeGroupBy === opt.id && <Check className="w-3.5 h-3.5 text-stone-700" />}
                      </button>
                    ))}
                  </div>
                </>
